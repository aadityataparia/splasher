...
const OnChangeSink = memo(({ onChange }) => {
    ...
    useEffect(() => {
        const isValid = Object.keys(errors).filter((k) => errors[k] !== undefined).length < 1
        if (isValid) {
            _onChange(values)
        } else {
            _onChange(null)
...
const FormBuilder = memo(
    ({ children = null, 'data-is-field': isField, onChange = null, style = {}, onSubmit, ...formikProps }) => {
        ...
        return (
            <Formik onSubmit={_onSubmit} {...formikProps}>
                {({ handleReset, handleSubmit }) => (
                    <Component style={style} onReset={handleReset} onSubmit={handleSubmit}>
                        {children}
...
export { FormBuilder, Field, SubmitButton, ResetButton, FormValueProvider, FieldValueChanger }
